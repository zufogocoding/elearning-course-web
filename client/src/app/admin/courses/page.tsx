...
const COVER_PRESETS = [
  ...
  {
    name: "UI/UX Design",
    url: "https://images.unsplash.com/photo-1561070791-26c113006238?w=800&auto=format&fit=crop",
  },
  {
    name: "Marketing",
...
export default function AdminCoursesPage() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        try {
          const res = await api.post("/api/courses/upload", {
            image: base64Data,
          });
  ...
  const openEditModal = (course: Course) => {
    ...
    setForm({
      ...
      discountPrice: course.discountPrice
          ? String(Number(course.discountPrice))
          : "",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!editCourse) {
        ...
        if (!res.ok) {
          setFormErrors(data.error || "Failed");
          return;
        }
      } else {
  ...
  const toggleStatus = async (course: Course) => {
    ...
    try {
      const res = await api.put(`/api/courses/${course.id}`, {
        status: newStatus,
      });
  ...
  return (
      <AdminLayout>
        <div className="p-6 py-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              <Search
                  className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 ${muted}`}
              />
              <input
                  id="course-search"
                  type="text"
            ...
            <div className="relative">
              ...
              <ChevronDown
                  className={`absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none ${muted}`}
              />
            </div>
          </div>
          ...
          <div className={`border rounded-2xl overflow-hidden ${card}`}>
            {loading ? (
                <div className="flex items-center justify-center py-20">
                  ...
                  <span className={`ml-3 text-sm font-medium ${muted}`}>
                Đang tải dữ liệu...
              </span>
                </div>
            ) : (
                <>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tr className={`border-b ${divider} ${sectionHdr}`}>
                        <th
                            className={`text-left px-4 py-3 text-xs font-semibold ${muted}`}
                        >
                          Khóa học
                        </th>
                        <th
                        ...
                        <th
                            className={`text-right px-4 py-3 text-xs font-semibold ${muted}`}
                        >
                          Hành động
                        </th>
                      </tr>
                      ...
                      {courses.length === 0 ? (
                          <tr>
                            <td
                                colSpan={6}
                                className={`px-4 py-16 text-center text-sm ${muted}`}
                            >
                              ...
                              >
                                <td className="px-4 py-3.5">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <p
                                          className={`font-semibold text-sm leading-tight ${text}`}
                                      >
                                        {course.title}
                                      </p>
                                ...
                                <td className="px-4 py-3.5">
                            <span
                                className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${levelBadge(course.level)}`}
                            >
                              {course.level === "beginner"
                                ...
                                <td className={`px-4 py-3.5 font-semibold ${text}`}>
                                  ...
                                  {course.discountPrice && (
                                      <span
                                          className={`ml-1.5 text-xs line-through ${muted}`}
                                      >
                                ...
                                <td className={`px-4 py-3.5 ${text}`}>
                                  {course._count.enrollments}
                                </td>

                                <td className="px-4 py-3.5">
                            <span
                                className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full ${statusBadge(course.status)}`}
                            >
                              {course.status === "published"
                                  ? "Đã xuất bản"
                                  : course.status === "draft"
                                  ...
                                  Xoá?
                                      ...
                                      <div className="flex items-center justify-end gap-1.5">
                                        ...
                                        <button
                                            ...
                                            className={`p-1.5 rounded-lg transition-all ${
                                                isDark
                                                    ? "bg-[#22263a] hover:bg-rose-500/20 text-[#a0aec0] hover:text-rose-400"
                                                    : "bg-slate-100 hover:bg-rose-50 text-slate-600 hover:text-rose-500"
                                            }`}
                                        >
                  ...
                  {pagination.totalPages > 1 && (
                      <div
                          className={`flex items-center justify-between px-4 py-3 border-t ${divider} ${sectionHdr}`}
                      >
                        ...
                        <div className="flex gap-1">
                          <button
                              ...
                              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                                  page <= 1 ? "opacity-40 cursor-not-allowed" : iconBtn
                              }`}
                          >
                            Trước
                          </button>
        ...
        {showAddModal && (
            ...
            >
              <div
                  className={`w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl border ${
                      isDark
                          ? "bg-[#13151f] border-[#252840]"
                          : "bg-white border-slate-200"
                  }`}
              >
                <div
                    className={`flex items-center justify-between px-6 py-4 border-b ${divider}`}
                >
                  <div className="flex items-center gap-3">
                    <h3 className={`text-base font-bold ${text}`}>
                      {editCourse ? "Sửa khóa học" : "Thêm khóa học mới"}
                    </h3>
                    ...
                    {editCourse && (
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <BookOpen className="w-3.5 h-3.5" />
                            Biên soạn giáo trình & quiz
                          </Link>

                          <Link
                              href={`/admin/courses/${editCourse.slug}/content`}
                              className="flex items-center gap-1.5 px-3 py-1 bg-blue-600/10 text-blue-400 border border-blue-500/20 hover:bg-blue-600/20 text-xs font-bold rounded-xl transition-all shadow-sm"
                          >
                            <FileText className="w-3.5 h-3.5" />
                            Quản lý nội dung
                          </Link>
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label
                          ...
                          className={`block text-xs font-semibold mb-1.5 ${muted}`}
                      >
                        Tên khóa học *
                      </label>
                      <input
                          id="course-title"
                          type="text"
                          placeholder="VD: Khóa học JavaScript toàn diện"
                          value={form.title}
                    ...
                    <div>
                      ...
                      >
                        Đường dẫn{" "}
                        {editCourse && (
                            <span className="text-amber-400">(chỉ đọc)</span>
                        )}
                      </label>
                      <input
                          id="course-slug"
                          ...
                          readOnly={!!editCourse}
                          className={`w-full px-4 py-3 border rounded-xl outline-none focus:ring-2 transition-all text-sm ${input} ${
                              editCourse ? "opacity-50 cursor-not-allowed" : ""
                          }`}
                      />
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-3">
                        {form.thumbnailUrl ? (
                            ...
                            <div
                                className={`aspect-video w-full rounded-2xl border flex items-center justify-center border-dashed ${
                                    isDark
                                        ? "bg-[#181a27] border-[#2d314d]"
                        ...
                        <div>
                          <label
                              className={`block text-[9px] font-bold uppercase tracking-wider mb-1 ${muted}`}
                          >
                  ...
                  <div>
                    <label className={`block text-xs font-semibold mb-2 ${muted}`}>
                      Trạng thái
                    </label>
                          ...
                          <button
                              ...
                              className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all border ${
                                  form.status === opt
                                      ...
                                      : isDark
                                          ? "border-[#252840] text-[#7a87a1] hover:border-[#3a3f55]"
                ...
                <div
                    className={`flex items-center justify-end gap-3 px-6 py-4 border-t ${divider}`}
                >
                  <button
