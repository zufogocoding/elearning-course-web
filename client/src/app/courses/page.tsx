...
import {
  Search, SlidersHorizontal, Star, ChevronDown, X,
  BookOpen, Users, BarChart2, ArrowLeft, ArrowRight
} from "lucide-react";
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-8">
          ...
          <p className={`text-sm ${muted}`}>
            Explore thousands of courses and level up your skills today.
          </p>
        </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mb-5">
              <p className={`text-sm font-medium ${muted}`}>
                Showing <span className={`font-bold ${text}`}>{mockCourses.length}</span> courses
                {searchQuery && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
              {mockCourses.map((course) => (
                ...
                >
                  {/* Thumbnail */}
                  <div className={`aspect-video w-full bg-gradient-to-br ${course.gradient} relative overflow-hidden`}>
                  ...
                  <div className="p-4 space-y-3">
                    <div>
                      <h3 className={`font-bold text-sm leading-snug line-clamp-2 ${text} group-hover:text-indigo-500 transition-colors`}>
                        {course.title}
                      </h3>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`text-xs ${muted}`}>
                        ({(course.reviews / 1000).toFixed(0)}k)
                      </span>
                    </div>
                    ...
                    <div className={`flex items-center justify-between pt-2 border-t ${divider}`}>
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className={`text-xs line-through ${subtle}`}>${course.originalPrice}</span>
                      </div>
                      <Link
